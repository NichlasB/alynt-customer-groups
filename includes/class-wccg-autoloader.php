...
class WCCG_Autoloader {
	...
	public function autoload( $class_name ) {
		...
		$file_paths = array(
			$this->get_file_path_from_class( $class_name ),
			$this->get_legacy_file_path_from_class( $class_name ),
		);
	...
	/**
	 * Convert a WCCG_ class name to its PHPCS-compliant filename.
	 *
	 * @since  1.0.0
	 * @param  string $class_name The class name to convert.
	 * @return string The expected filename (e.g. class-wccg-admin-pricing-rules.php).
	 */
	...
	/**
	 * Convert a WCCG_ class name to the legacy filename used by older plugin files.
	 *
	 * @since  1.0.0
	 * @param  string $class_name The class name to convert.
	 * @return string Legacy filename without the WCCG_ prefix.
	 */
	private function get_legacy_file_name_from_class( $class_name ) {
	...
	private function get_file_path_from_class( $class_name ) {
		...
		if ( strpos( $class_name, 'WCCG_Admin_' ) === 0 || 'WCCG_Admin' === $class_name ) {
	...
	/**
	 * Resolve the absolute legacy file path for a WCCG_ class.
	 *
	 * @since  1.0.0
	 * @param  string $class_name The class name to resolve.
	 * @return string Absolute path to the legacy class file.
	 */
	private function get_legacy_file_path_from_class( $class_name ) {
		$file_name = $this->get_legacy_file_name_from_class( $class_name );
