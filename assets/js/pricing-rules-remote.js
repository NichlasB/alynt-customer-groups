(function($) {
    ...
    const initPricingRuleModal = () => {
        ...
        const closeModal = () => {
            ...
            hydrateRemoteSelect($productSelect, []);
            hydrateRemoteSelect($categorySelect, []);
            $modal.removeAttr('aria-busy');
        };

        if (hasSelect2 && $productSelect.hasClass('select2-hidden-accessible')) {
            $productSelect.select2('destroy');
        }
        if (hasSelect2 && $categorySelect.hasClass('select2-hidden-accessible')) {
            $categorySelect.select2('destroy');
        }
